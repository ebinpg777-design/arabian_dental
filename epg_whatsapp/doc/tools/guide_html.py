...
name = sys.argv[3] if len(sys.argv) > 3 else "USER_GUIDE.md"
title = sys.argv[4] if len(sys.argv) > 4 else "WhatsApp — Setup and User Guide"
src = open(f"{doc}/{name}", encoding="utf-8").read()
...
open(out, "w", encoding="utf-8").write(f"<!doctype html><html><head><meta charset='utf-8'><title>{title}</title><style>{css}</style></head><body>{body}</body></html>")
