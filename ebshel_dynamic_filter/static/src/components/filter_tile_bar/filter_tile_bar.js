...
import { tileDomain } from "@ebshel_dynamic_filter/core/tile_domain";
...
export class FilterTileBar extends Component {
    static template = "ebshel_dynamic_filter.FilterTileBar";
    ...
    settingKey(name) {
        return `ebshel_dynamic_filter.${name}.${this.resModel}`;
    }
    ...
    clearTileFacets() {
        ...
        const droppedGroupBys = this.clearTileGroupBys();
        ...
        if (droppedGroupBys && !groupIds.size) {
            searchModel._notify();
        }
    }

    // ------------------------------------------------------------------
    // Selection -> group-by facets
    //
    // A tile may carry a field to group the view by. The grouping is a search
    // item of its own (a filter facet and a group-by facet cannot share one),
    // built the way the search model's own `createNewGroupBy` builds it but
    // pushed into the query silently: the filter facet that follows triggers
    // the one reload for both.
    // ------------------------------------------------------------------

    /** Take this bar's group-by items out of the query; true when any were. */
    clearTileGroupBys() {
        const searchModel = this.env.searchModel;
        const before = searchModel.query.length;
        searchModel.query = searchModel.query.filter((queryElem) => {
            const item = searchModel.searchItems[queryElem.searchItemId];
            return !(item && item.isFilterTileGroupBy);
        });
        return searchModel.query.length !== before;
    }

    /** Group the view the way `def` asks for, unless it already is. */
    addTileGroupBy(def) {
        const searchModel = this.env.searchModel;
        const fieldName = def.group_by;
        const field = fieldName && searchModel.searchViewFields[fieldName];
        if (!field) {
            return false; // the view does not know the field: filter only
        }
        const isDate = ["date", "datetime"].includes(field.type);
        const interval = isDate ? def.group_by_interval || "month" : undefined;
        const alreadyGrouped = searchModel.query.some((queryElem) => {
            const item = searchModel.searchItems[queryElem.searchItemId];
            return (
                item &&
                ["groupBy", "dateGroupBy"].includes(item.type) &&
                item.fieldName === fieldName &&
                (!isDate || queryElem.intervalId === interval)
            );
        });
        if (alreadyGrouped) {
            return false;
        ...
        const firstGroupBy = Object.values(searchModel.searchItems).find(
            (item) => item.type === "groupBy"
        );
        const id = searchModel.nextId++;
        searchModel.searchItems[id] = {
            type: isDate ? "dateGroupBy" : "groupBy",
            ...
            id,
            custom: true,
            // Keep it out of the Group By dropdown; it stays a removable facet.
            invisible: "True",
            isFilterTileGroupBy: true,
            tileKey: def.key,
            ...(isDate ? { defaultIntervalId: interval } : {}),
        };
        searchModel.query.push(isDate ? { searchItemId: id, intervalId: interval } : { searchItemId: id });
        return true;
    }

    /**
     * A grouping only makes sense while the tile that brought it is applied:
     * when the user drops the tile facet, its group-by facet goes with it.
     */
    dropOrphanGroupBys() {
        const searchModel = this.env.searchModel;
        if (!searchModel || this.droppingGroupBys) {
            return;
        }
        const active = new Set(this.activeKeys);
        const orphaned = searchModel.query.some((queryElem) => {
            const item = searchModel.searchItems[queryElem.searchItemId];
            return item && item.isFilterTileGroupBy && !active.has(item.tileKey);
        });
        if (!orphaned) {
            return;
        }
        this.droppingGroupBys = true;
        try {
            searchModel.query = searchModel.query.filter((queryElem) => {
                const item = searchModel.searchItems[queryElem.searchItemId];
                return !(item && item.isFilterTileGroupBy && !active.has(item.tileKey));
     ...
     * the answer per domain string. A tile it refuses is shown greyed and
     * never applied: applying one used to take the whole view down, every
     * time it was opened, for everybody. (client, 2026-09-10)
     */
    tileIsBroken(def) {
        if (def.broken) {
            return true;
    ...
    applySelection(keys) {
        ...
        if (defs.some((def) => def.ignore_filters) && searchModel.clearQuery) {
            searchModel.clearQuery();
        }
        const prefilters = defs.map((def) => ({
        ...
        // Group-bys first, silently: the filters below notify once for both.
        defs.forEach((def) => this.addTileGroupBy(def));
