...
import { tileDomain, tileGroupBySpec } from "@ebshel_dynamic_filter/core/tile_domain";
...
export class TileOverview extends Component {
    static template = "ebshel_dynamic_filter.TileOverview";
    ...
    openTile(section, tile) {
        const groupBy = tileGroupBySpec(tile);
        this.action.doAction({
            ...
            // A grouping tile opens its records grouped, as on the ribbon.
            context: groupBy ? { group_by: [groupBy] } : {},
        });
    }
...
registry.category("actions").add("ebshel_dynamic_filter.overview", TileOverview);
