...
# Field types a tile may group the view by when it is clicked: what the search
# bar's own "Group By" menu accepts, so the facet it creates is an ordinary one.
GROUP_BY_TYPES = (
    'many2one', 'many2many', 'selection', 'boolean', 'char', 'integer', 'date', 'datetime',
)
GROUP_BY_INTERVALS = [
...
EXPORT_FIELDS = (
    ...
    'alert_operator', 'alert_value', 'alert_message', 'alert_notify', 'group_by_interval',
    'ignore_filters',
)
...
class FilterTile(models.Model):
    ...
    # --- group by --------------------------------------------------------
    # A tile is a filter; this makes it a filter *and* a grouping. Clicking
    # "Late" on a tile grouped by salesperson shows who is late, in one click,
    # as two ordinary facets the user can drop one at a time.
    group_by_field_id = fields.Many2one(
        'ir.model.fields', string='Group View By', ondelete='set null',
        domain="[('model_id', '=', model_id), ('ttype', 'in',"
               " ['many2one', 'many2many', 'selection', 'boolean', 'char', 'integer',"
               " 'date', 'datetime']), ('store', '=', True)]",
        help="Clicking the tile also groups the view by this field - status, salesperson, "
             "month... Empty: the tile only filters.")
    group_by_field_name = fields.Char(
        related='group_by_field_id.name', string='Group By Field Name')
    group_by_field_type = fields.Selection(
        related='group_by_field_id.ttype', string='Group By Field Type')
    group_by_interval = fields.Selection(
        GROUP_BY_INTERVALS, string='Per', default='month', required=True,
        help="How a date field is bucketed when the view is grouped by it.")
    ...
    def _is_tile_manager(self):
        return self.env.user.has_group('ebshel_dynamic_filter.group_tile_manager')
    ...
    def _check_group_by(self):
        for tile in self:
            field = tile.sudo().group_by_field_id
            if not field:
                continue
            if field.model != tile.model_name or not field.store or field.ttype not in GROUP_BY_TYPES:
                raise ValidationError(self.env._(
                    'Tile "%(name)s" cannot group %(model)s by "%(field)s": pick a stored '
                    'field of that model the view can be grouped by.',
                    name=tile.name, model=tile.model_name, field=field.field_description))
    ...
    def _tile_payload(self):
        ...
        return {
            ...
            # What a click groups the view by: '' for a plain filter tile.
            'group_by': fields_sudo.group_by_field_id.name or '',
            'group_by_label': fields_sudo.group_by_field_id.field_description or '',
            'group_by_type': fields_sudo.group_by_field_id.ttype or '',
            'group_by_interval': self.group_by_interval or 'month',
    ...
    def action_open_tiles(self):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id(
            'ebshel_dynamic_filter.action_filter_tile')
    ...
    def _alert_audience(self):
        ...
        managers = self.env.ref(
            'ebshel_dynamic_filter.group_tile_manager', raise_if_not_found=False)
    ...
    def export_tiles(self, model_name=None, tile_ids=None):
        ...
        for tile in tiles:
            ...
            values.update({
                ...
                'trend_field': fields_sudo.trend_field_id.name or '',
                'breakdown_field': fields_sudo.breakdown_field_id.name or '',
                'group_by_field': fields_sudo.group_by_field_id.name or '',
                'groups': [
                    xmlid for xmlid in fields_sudo.group_ids.get_external_id().values() if xmlid
                ],
            })
            payload.append(values)
        return {'version': 1, 'generator': 'ebshel_dynamic_filter', 'tiles': payload}
    ...
    def import_tiles(self, payload, replace=False):
        ...
        for entry in payload['tiles']:
            ...
            for key, target in (('measure', 'measure_field_id'),
                                ...
                                ('group_by_field', 'group_by_field_id')):
