...
```bash
odoo-bin -d <database> -i dynamic_filter_tiles
```
...
instances/odoo19/start.sh -d odoo19_tiles -i dynamic_filter_tiles --stop-after-init --no-http

# run it
instances/odoo19/start.sh            # http://localhost:1919

# apply changes
instances/odoo19/start.sh -d odoo19_tiles -u dynamic_filter_tiles --stop-after-init --no-http
```
