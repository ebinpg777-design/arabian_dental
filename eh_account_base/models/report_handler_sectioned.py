...
class EhAccountDynamicReportSectionedHandler(models.AbstractModel):
    ...
    def _resolve_comparison_dates(self, mode, date_from, date_to):
        ...
        Modes supported:
        * 'previous_period' compares against the window of the same length
          ending the day before date_from. A window of whole calendar months
          (first day of a month to last day of a month) shifts by the same
          number of MONTHS, so March (31 days) compares against all of
          February and Q2 against Q1; any other window shifts by its day
          count.
        * 'previous_year' shifts both ends back by one calendar year. A
        ...
        if mode == 'previous_period':
            months = self._eh_whole_month_span(date_from, date_to)
            if months:
                # A day-count shift misaligns month windows (March's 31 days
                # back from Mar 1 starts on Jan 29); step whole months back.
    ...
    @staticmethod
    def _eh_whole_month_span(date_from, date_to):
        """Number of calendar months covered when [date_from, date_to] runs
        from the first day of a month to the last day of a month, else 0."""
        if not date_from or not date_to or date_from > date_to:
            return 0
        if date_from.day != 1 or (date_to + timedelta(days=1)).day != 1:
            return 0
    ...
    def _cash_basis_grouped_totals(
        ...
        options = options or {}
        # Mirror the accrual SQL path (_fetch_grouped_account_totals +
        # MoveLineQuery): empty account_types means no type filter, open date
        # bounds add no condition (the balance-sheet snapshot passes no
        # date_from), cancelled entries are always excluded, and the analytic
        # filters apply.
        domain = [
        ...
        if options.get('analytic_account_ids'):
            # analytic_distribution's search splits multi-account keys
            # ("3,5"), matching MoveLineQuery.where_analytic_accounts.
            domain.append(('analytic_distribution', 'in', [
                int(i) for i in options['analytic_account_ids']]))
        if options.get('analytic_plan_ids'):
            plan_account_ids = self.env['account.analytic.account'].sudo() \
                .with_context(active_test=False).search([
                    ('plan_id', 'child_of', [
                        int(i) for i in options['analytic_plan_ids']]),
                ]).ids
            if not plan_account_ids:
                return []
            domain.append(('analytic_distribution', 'in', plan_account_ids))
        ...
        for line in lines:
            ...
            if line.account_id.id not in meta:
                # account.account code is company-dependent: read it for the
                # line's own company, not whichever company is active.
                account = line.account_id.with_company(line.company_id)
                meta[line.account_id.id] = (account.code, account.name)
    ...
    def _eh_move_paid_fraction(self, move, date_to):
        ...
        reconciled = sum(
            ...
            if p.max_date and (not date_to or p.max_date <= date_to))
