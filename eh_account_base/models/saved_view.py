...
class EhReportSavedView(models.Model):
    ...
    def _eh_can_manage_others(self):
        return self.env.su or self.env.user.has_group(
            'eh_account_base.group_eh_manager')

    @api.model_create_multi
    def create(self, vals_list):
        # A user may only create views owned by themselves; otherwise a view
        # could be planted (and shared) in another user's name.
        if not self._eh_can_manage_others():
            for vals in vals_list:
                owner = vals.get('user_id')
                if owner and owner != self.env.uid:
                    raise AccessError(_(
                        "You can only create saved views owned by yourself."))
        return super().create(vals_list)

    def write(self, vals):
        # Owner-only edit. The record rule lets every company user READ a
        # shared view, and ACL grants write on the model, so without this
        # guard anyone could overwrite (or take over) a colleague's shared
        # view. Managers and the superuser keep full control.
        if not self._eh_can_manage_others():
            foreign = self.filtered(
                lambda r: r.user_id and r.user_id.id != self.env.uid)
            if foreign:
                raise AccessError(_(
                    "Only the owner can edit a saved view (%s).",
                ) % ', '.join(foreign.mapped('name')))
            if vals.get('user_id') and vals['user_id'] != self.env.uid:
                raise AccessError(_(
                    "You cannot transfer a saved view to another user."))
