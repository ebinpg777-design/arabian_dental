...
whenever a report-relevant field (amount, account, date, partner or analytic
distribution) changes on a line whose parent move is posted, and only then,
so unrelated writes (labels, narration, reconciliation bookkeeping) do not
over-bump.
...
# Fields whose change on a posted line alters a cached report without moving
# an amount: the partner (partner ledger, aged receivable/payable, customer
# statement) and the analytic distribution (every analytic-filtered report).
# They invalidate the reporting cache but deliberately stay out of the seal
# set above, which guards the figures of a sealed sub-ledger entry only.
_EH_CACHE_LINE_FIELDS = _EH_MATERIAL_LINE_FIELDS | frozenset({
...
class AccountMove(models.Model):
    ...
    def write(self, vals):
        ...
        if 'state' in vals and self:
            ...
            changed_ids = []
        # A partner change on a posted move re-derives its lines' partner by
        # recompute (no line write() fires), so bump for it here.
        if 'partner_id' in vals and self:
            changed_ids += [
                m.id for m in self
                if m.state == 'posted' and m.id not in changed_ids
            ]
...
class AccountMoveLine(models.Model):
    ...
    def write(self, vals):
        ...
        if self and _EH_CACHE_LINE_FIELDS.intersection(vals):
            posted = self.filtered(lambda line: line.move_id.state == 'posted')
            bump_company_ids = set(posted.mapped('company_id.id'))
        result = super().write(vals)
        if bump_company_ids:
