...
class EhAccountReportExecution(models.Model):
    ...
    def start_execution(self, report_code, name, options, company_ids, result_format='json'):
        ...
        # Audit rows are created ONLY here, as superuser: users hold read-only
        # access to this model and create() refuses any non-superuser call,
        # so a row served by find_cached() is always engine-produced and can
        # never be a payload planted over RPC. executed_by still records the
        # real requesting user (sudo keeps the uid).
    ...
    @api.model
    def _options_hash(self, options):
        """SHA-256 cache key of an options dict for the acting language.

        Account names (and every other translated label in a payload) are
        rendered in env.lang, so two renders of the same options in
        different languages are different payloads; the language is folded
        into the hash so a cached French payload is never served to an
        English user. The canonical options stay order-insensitive.
        """
        ...
        return self._hash_string(
            json.dumps(keyed, sort_keys=True, default=str))
    ...
    def write(self, vals):
        ...
        # a historical row, defeating the "append-only, hashed" promise. A
        # context flag is forgeable over RPC, so provenance is proven by
        # env.su alone: only the engine's own sudo() writers
        # (complete_execution / fail_execution / the cache-hit pointer) pass.
        if not self.env.su:
            raise UserError(_(
    ...
    def create(self, vals_list):
        # Rows are produced only by start_execution() (as superuser). A
        # user-created row with state 'done', a matching options_hash,
        # move_version_at_start and company_ids_key would otherwise be served
        # by find_cached() to every user rendering that report.
        if not self.env.su:
            raise AccessError(_(
                "Report execution audit rows are created only by the "
                "reporting engine."
            ))
    ...
    def _unlink_immutable_audit(self):
        if not (self.env.su
                and self.env.context.get('eh_internal_audit_unlink')):
            raise UserError(_(
