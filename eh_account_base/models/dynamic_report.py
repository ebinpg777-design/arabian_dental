...
class EhAccountDynamicReport(models.Model):
    ...
    def _eh_apply_presentation_currency(self, payload, options, company_ids):
        ...
        if not target:
            return payload
        # A handler that already consolidated through a CurrencyTable (the
        # Trial Balance's per-company rate join) discloses it in its meta.
        # Its figures are already in the presentation currency; converting
        # them again here would apply the rate twice.
        meta = payload.get('meta') or {}
        if meta.get('presentation_currency_id') or meta.get('multi_currency'):
            return payload
        company = (
        ...
        if not source or target == source:
            return payload
        # This restatement applies ONE company-to-target rate to every cell.
        # When the scope spans companies with different currencies the cells
        # are not all in `source`, so a single rate would silently misstate
        # every other company's figures. Refuse instead.
        scope_currencies = self.env['res.company'].sudo().browse(
            [int(c) for c in (company_ids or [company.id])]
        ).mapped('currency_id')
        if len(scope_currencies) > 1:
            raise UserError(_(
                "Report %(code)s cannot be restated into %(currency)s: the "
                "selected companies keep their books in different currencies "
                "(%(currencies)s) and this report does not consolidate "
                "per-company exchange rates. Select companies sharing one "
                "currency, or clear the presentation currency.",
                code=self.code, currency=target.name,
                currencies=', '.join(scope_currencies.mapped('name')),
            ))
    ...
    def render(self, options, result_format='json', use_cache=True):
        ...
        * from_cache: True when served from a prior execution payload.
        ...
        options_hash = Execution._options_hash(options)

        # Cache freshness is keyed on eh_move_version, which only moves when a
        # POSTED entry changes (state transition or material line edit).
        # Draft entries are created and edited without bumping it, so a
        # report that includes drafts can never be trusted from cache.
        if not options.get('posted_only', True):
            use_cache = False
    ...
    def export_xlsx_attachment(self, options):
        ...
        attachment = self.env['ir.attachment'].create({
            ...
            # Deliberately NOT linked to this report record: the report
            # definition is readable by every EH user in every company, so a
            # linked attachment (ledger figures) could be downloaded by id by
            # anyone. An unlinked attachment is readable only by its creator
            # (ir.attachment access rule), which is exactly the downloader.
            'res_model': False,
            'res_id': False,
        })
        return {
