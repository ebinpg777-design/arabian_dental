...
class TestMoveLineQueryShape(EhAccountUnitTestCase):
    ...
    def test_where_analytic_accounts_uses_jsonb_match(self):
        ...
        # Keys are split into account ids (so a combined "3,5" key matches)
        # and compared by array overlap against the requested ids.
        self.assertIn('regexp_split_to_array', sql.code)
        self.assertIn('aml.analytic_distribution', sql.code)
