...
class EhAccountIntegrationTestCase(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        # Run every integration test on a dedicated, freshly created company
        # instead of mutating env.company. On a database that already carries
        # real journal items (a staging copy, a production clone) the active
        # company cannot have its currency changed ("You cannot change the
        # currency of the company since some journal items already exist"),
        # and any figure the tests assert would be polluted by the real
        # ledger. A fresh company isolates both: its ledger is empty, its
        # currency can be pinned, and every report is scoped to it.
        cls.company = cls._eh_setup_test_company()
    ...
    @classmethod
    def _eh_setup_test_company(cls):
        """Create an isolated USD company with the generic chart of accounts
        and make it the active company of the test environment.

        The currency is pinned to USD so currency-sensitive tests behave
        identically across series (Odoo 16 defaults a company to EUR, 17+ to
        USD) and regardless of the host database's own company currency.
        Setting it is safe here because the new company has no journal item.
        The generic chart mirrors what a fresh database provisions for its
        main company (receivable/payable defaults, sale/purchase/bank/misc
        journals, exchange-difference accounts), which the fixtures below
        extend. The superuser is moved onto the company and the environment
        carries it in allowed_company_ids, so env.company, record defaults
        (company_id, currency rates) and users created by tests all resolve
        to it.
        """
        ...
        company = cls.env['res.company'].sudo().create({
            'name': 'EH Accounting Test Company',
            'currency_id': usd.id,
        })
        ChartTemplate = cls.env['account.chart.template']
        if 'generic_coa' in ChartTemplate._get_chart_template_mapping():
            ChartTemplate.try_loading(
                'generic_coa', company=company, install_demo=False)
        # The chart template may carry its own currency; the explicit pin
        # wins (still safe: no journal item exists on this company yet).
        if company.currency_id != usd:
            company.sudo().write({'currency_id': usd.id})
        if ('account_fiscal_country_id' in company._fields
                and not company.account_fiscal_country_id):
        ...
        cls.env = cls.env(context=dict(
            cls.env.context, allowed_company_ids=[company.id]))
    ...
    def _ensure_account(env, code, name, account_type):
        ...
        if existing:
            # The generic chart already uses some fixture codes with another
            # type (1010 is "Current Assets", 5000 "Cost of Goods Sold" as a
            # direct cost). Tests rely on the type they ask for, and the
            # dedicated test company has no entries yet, so align it.
            fix = {}
            if existing.account_type != account_type:
                fix['account_type'] = account_type
            if account_type in reconcilable and not existing.reconcile:
                fix['reconcile'] = True
            if fix:
                existing.sudo().write(fix)
            return existing
        vals = {
            'code': code,
            'name': name,
        ...
        if account_type in reconcilable:
            vals['reconcile'] = True
