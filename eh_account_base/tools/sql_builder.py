...
class MoveLineQuery:
    ...
    def where_analytic_accounts(self, analytic_account_ids):
        ...
        analytic_distribution is a jsonb keyed by analytic account id (as
        string) carrying the percentage allocated. A key names ONE account
        ("3") or, with several analytic plans, a comma-separated combination
        ("3,5"), so an exact-key match (?|) misses every combined key. Split
        all keys into account ids and test for overlap, the same expression
        core's analytic_distribution search uses (and its GIN index covers).
        The percentage threshold check is left to callers that need it via
        where_raw().
        """
        ...
        self._wheres.append(SQL(
            r"""regexp_split_to_array(jsonb_path_query_array(aml.analytic_distribution, '$.keyvalue()."key"')::text, '\D+') && %s""",
            keys,
        ))
        return self
    ...
    def where_analytic_plans(self, plan_ids):
        ...
        and passes through where_analytic_accounts. A plan's sub-plans are
        included (an account on "Projects / Internal" belongs to "Projects"),
        resolved through account_analytic_plan.parent_path.
        """
        ...
        cr = self.env.cr
        cr.execute(
            "SELECT parent_path FROM account_analytic_plan WHERE id IN %s",
            (ids,),
        )
        path_patterns = [r[0] + '%' for r in cr.fetchall() if r[0]]
        if not path_patterns:
            self._wheres.append(SQL("FALSE"))
            return self
        cr.execute(
            "SELECT acc.id FROM account_analytic_account acc "
            "JOIN account_analytic_plan plan ON plan.id = acc.plan_id "
            "WHERE plan.parent_path LIKE ANY(%s)",
            (path_patterns,),
        )
