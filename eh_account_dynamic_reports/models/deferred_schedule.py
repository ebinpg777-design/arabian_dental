...
class EhDeferredScheduleBase(models.AbstractModel):
    ...
    # eh.asset states that are not a live deferral: never listed on the
    # schedule, never drillable.
    EXCLUDED_ASSET_STATES = ('draft', 'cancel', 'cancelled')

    @api.model
    def _asset_scope_domain(self, company_ids):
        """Domain of the deferrals this schedule lists.

        Shared by the schedule build and the drilldown, so a row can only
        open entries of a deferral the schedule itself would show: bounded
        by deferred_type and company, and excluding draft / cancelled
        states when the model carries a state field.
        """
        domain = [
            ('deferred_type', '=', self.DEFERRED_TYPE),
            ('company_id', 'in', [int(c) for c in company_ids]),
        ]
        if (self._eh_asset_available()
                and 'state' in self.env['eh.asset']._fields):
            domain.append(
                ('state', 'not in', list(self.EXCLUDED_ASSET_STATES)))
        return domain

    @api.model
    def _discover_assets(self, company_ids):
        """Return the eh.asset recordset in scope for this deferred type
        (see _asset_scope_domain)."""
        return self.env['eh.asset'].sudo().search(
            self._asset_scope_domain(company_ids))
    ...
    def get_drilldown_action(self, options, line_id):
        ...
        company_ids = options.get('company_ids') or [self.env.company.id]
        try:
            # Searched, not browsed: an id outside this schedule's type,
            # companies or live states opens nothing.
            asset = self.env['eh.asset'].sudo().search(
                self._asset_scope_domain(company_ids)
                + [('id', '=', asset_id)], limit=1)
            if not asset:
                return None
        ...
        if not move_ids:
            return None
        domain = self._drilldown_move_domain(
            options, move_ids, company_ids, date_to)
        return {
    ...
    @api.model
    def _drilldown_move_domain(self, options, move_ids, company_ids, date_to):
        """Journal item domain for a deferral row's drilldown.

        Mirrors compute(): the same company scope, and posted_only read
        with the same default (True). Posted only opens posted entries;
        otherwise draft entries open too, never cancelled ones.
        """
        domain = [
        ...
        if bool(options.get('posted_only', True)):
