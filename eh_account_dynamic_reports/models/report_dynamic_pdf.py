...
class EhAccountDynamicReportPdf(models.Model):
    ...
    def export_pdf_attachment(self, options):
        ...
        attachment = self.env['ir.attachment'].create({
            ...
            # Not linked to the report record: that definition is readable by
            # every EH user in every company, which would expose the ledger
            # PDF to anyone guessing the id. Unlinked = creator-only access.
            'res_model': False,
            'res_id': False,
        })
        return {
