...
class EhAgedBaseHandler(models.AbstractModel):
    ...
    @api.model
    def eh_sales_routes(self, limit=200):
        """The Sales Routes for the report's filter, read with sudo.

        The filter used to be populated by a plain searchRead on crm.team from the
        browser: an accountant without rights on the sales models got an AccessError,
        the viewer swallowed it, the list came back empty and the WHOLE filter silently
        disappeared. A route is a label on a clinic, not sales data, so it is served
        from here instead. (client, 2026-08-21)
        """
        if 'crm.team' not in self.env:
            return []
        teams = self.env['crm.team'].sudo().search([], limit=limit, order='name')
        return [{'id': team.id, 'name': team.name or ''} for team in teams]
    ...
    def compute(self, options):
        ...
        return {
            ...
            'meta': {
                ...
                'reconcile_state': reconcile_state,
                'group_by_route': bool(options.get('group_by_route')),
                'team_ids': [int(t) for t in options.get('team_ids') or []],
            },
        }
    ...
    def _fetch_open_lines(
        ...
        # Sales Route filter: a route is a set of clinics, so it becomes a partner
        # restriction here — the clinic's own route, or its parent company's.
        team_ids = [int(t) for t in (options or {}).get('team_ids') or []]
        if team_ids and 'team_id' in self.env['res.partner']._fields:
            routed = self.env['res.partner'].with_context(active_test=False).search([
                '|', ('team_id', 'in', team_ids),
                '&', ('team_id', '=', False), ('commercial_partner_id.team_id', 'in', team_ids),
            ])
            # an impossible id when the route has no clinics: the report goes empty,
            # rather than silently showing everyone
            query.where_partners(routed.ids or [-1])
        if posted_only:
    ...
    def _build_lines_and_totals(
        ...
        sorted_partner_ids = sorted(
            ...
            key=lambda pid: str(partner_buckets[pid].get('partner_name') or '').lower(),
        )
        routes = {}
        if group_by_route:
            # The clinic's Sales Route (crm.team, held on the partner by sale_custom).
            # Read with sudo and behind a guard: the route name is a label, not
            # confidential, and a reader without rights on crm.team - or a route from a
            # company the reader is not in - must not turn the whole report into a
            # server error. Anything unreadable simply lands under "No Route".
            # (client, 2026-08-20: "error when clicking group by sales route")
            no_route = _("No Route")
            try:
                partners = self.env['res.partner'].sudo().browse(
                    list(partner_buckets)).exists()
                has_team = 'team_id' in self.env['res.partner']._fields
                for partner in partners:
                    team = partner.team_id if has_team else False
                    team = team or (partner.commercial_partner_id.team_id
                                    if has_team else False)
                    name = (team.name or '').strip() if team else ''
                    routes[partner.id] = (team.id if team else 0, name or no_route)
            except Exception:                                       # noqa: BLE001
                _logger.warning(
                    "Aged report: sales routes unreadable, grouping everything under "
                    "%s", no_route, exc_info=True)
                routes = {}
            sorted_partner_ids.sort(key=lambda pid: (
                str(routes.get(pid, (0, no_route))[1] or no_route).lower(),
                str(partner_buckets[pid].get('partner_name') or '').lower()))

        lines = []
        route_rows = {}    # route key -> its subtotal row (built as partners stream by)
        for partner_id in sorted_partner_ids:
            data = partner_buckets[partner_id]
            row_total = round(sum(data[key] for key in bucket_keys), 2)
            if not show_zero and row_total == 0.0:
                # Nothing outstanding: buckets that offset to zero are as much
                # "nothing owed" as an empty row (client, 2026-08-19).
                continue
            ...
            if group_by_route:
                route_key, route_name = routes.get(partner_id, (0, _("No Route")))
                route_name = route_name or _("No Route")
                if route_key not in route_rows:
                    row = {
                        'id': "route-%s" % route_key,
                        'name': route_name,
                        'level': 0,
                        'columns': [
                            {'expression_label': key, 'value': 0.0}
                            for key in bucket_keys
                        ] + [{'expression_label': 'total', 'value': 0.0}],
                        'unfoldable': False,
                        'meta': {'kind': 'route_header', 'team_id': route_key},
                    }
                    route_rows[route_key] = row
                    lines.append(row)
                header = route_rows[route_key]
                for index, key in enumerate(bucket_keys):
                    header['columns'][index]['value'] = round(
                        header['columns'][index]['value'] + data[key], 2)
                header['columns'][-1]['value'] = round(
                    header['columns'][-1]['value'] + row_total, 2)

            line = {
                'id': "partner-%s" % partner_id,
                'name': data['partner_name'] or "(no name)",
                'level': 2 if group_by_route else 1,
                'columns': columns,
                'unfoldable': False,
