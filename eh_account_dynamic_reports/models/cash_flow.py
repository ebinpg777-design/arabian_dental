...
Report filters (direct method): journal and partner filters apply to every
journal item measured. The account filter is split by account. Cash
accounts named in it (cash types and configured equivalents) narrow the
cash position to those accounts; cash legs of the same entries on other
cash accounts (a transfer with one side filtered out) are not activity and
are shown on their own "Transfers with cash accounts outside the filter"
line, which enters the Balance Check. Any other account in the filter
selects entries. Analytic filters select entries too: cash legs never carry
an analytic distribution, so matching them line by line would empty the
statement. An entry is in scope when any of its lines matches; its cash
legs and all of its counterparts (including lines on other analytic
accounts) are then measured in full, which keeps
Closing = Opening + Net Change + FX + transfers exact under every filter.
...
P&L account, plus back the income tax expense identified by the EH Income
Tax Paid tag and, when that item has a tagged clearing account, the
company's Tax Expense account mapping), adjusts for finance costs and
investment income (so the disclosed interest and dividend flows can be
presented gross further down; each account is bridged once at most), adds
back non-cash charges resolved from the EH Non-Cash account tags
(depreciation / amortisation, impairment, provisions, unrealised FX, fair
value), applies the working-capital deltas measured from the balance-sheet
movement of the receivable / payable / other-current / prepayment groups,
and closes with the IAS 7.31-35 disclosed flows (interest paid, income
taxes paid, and whichever disclosed items the presentation policy assigns
to operating).
...
* Credit cards (`liability_credit_card`) are treated as financing rather
  than cash.
* The entry-level filter scoping and the transfers line describe the
  direct method. The indirect method's profit, add-back and
  working-capital figures still apply the account and analytic filters
  line by line, so under those filters its operating total need not tie
...
class EhCashFlowHandler(models.AbstractModel):
    ...
    # Prepayments are short-term working capital (cash paid ahead of an
    # operating expense), so they sit in operating, not investing.
    INVESTING_TYPES = (
    ...
    # same operating perimeter (prepayments are working capital in both).
    WC_TYPES = (
        'asset_receivable', 'liability_payable',
        'asset_current', 'asset_prepayments', 'liability_current',
    )
    ...
    def compute(self, options):
        ...
        # Cash account filter: legs on cash accounts outside the filter.
        cash_transfers = self._fetch_cash_transfers(
            move_ids=cash_move_ids, company_ids=company_ids,
            posted_only=posted_only, options=options,
            equivalent_ids=equivalent_ids,
        )
        ...
        #            (+ transfers with cash accounts outside the filter).
        balance_check = round(
        ...
        # Shown only under a cash account filter that splits transfers.
        if cash_transfers:
            lines.append(self._computed_line(
                'cash_transfers_outside_filter',
                _("Transfers with cash accounts outside the filter"),
                cash_transfers, kind='cash_transfers',
            ))
        lines.append(self._computed_line(
        ...
        if cash_transfers:
            totals['cash_transfers_outside_filter'] = cash_transfers
    ...
    def _split_account_filter(self, options, equivalent_ids=()):
        """Split options['account_ids'] into (cash_ids, entry_ids).

        Cash accounts (CASH_TYPES plus configured equivalents) scope the
        cash legs themselves. Every other id selects journal entries
        (_apply_entry_scope). Both tuples are empty when no account
        filter is set.
        """
        ids = {int(i) for i in ((options or {}).get('account_ids') or ())}
        if not ids:
            return (), ()
        equivalents = set(equivalent_ids or ())
        cash_ids = {
            account.id
            for account in self.env['account.account'].sudo().browse(
                sorted(ids)).exists()
            if account.account_type in self.CASH_TYPES
            or account.id in equivalents
        }
        return tuple(sorted(cash_ids)), tuple(sorted(ids - cash_ids))

    def _apply_entry_scope(self, query, options, entry_account_ids=()):
        """Select whole journal entries by their lines.

        Used for the analytic filters and the non-cash part of the account
        filter. Cash legs carry no analytic distribution and never sit on
        a non-cash account, so these filters are evaluated on the entry:
        it is in scope when any of its lines matches (each dimension
        separately, combined with AND), and its cash legs are then
        measured in full. With none of these filters the query is left
        untouched.
        """
        options = options or {}
        if entry_account_ids:
            query.where_raw(SQL(
                "aml.move_id IN (SELECT scope.move_id"
                " FROM account_move_line scope"
                " WHERE scope.account_id IN %s)",
                tuple(entry_account_ids),
            ))
        analytic_sets = []
        if options.get('analytic_account_ids'):
            analytic_sets.append(
                [int(i) for i in options['analytic_account_ids']])
        if options.get('analytic_plan_ids'):
            # Same plan -> account resolution as
            # MoveLineQuery.where_analytic_plans (every account of the plan,
            # archived included), through the ORM so pending writes flush.
            analytic_sets.append(
                self.env['account.analytic.account'].sudo().with_context(
                    active_test=False,
                ).search([
                    ('plan_id', 'in',
                     [int(i) for i in options['analytic_plan_ids']]),
                ]).ids)
        for analytic_ids in analytic_sets:
            if not analytic_ids:
            ...
            query.where_raw(SQL(
                "aml.move_id IN (SELECT scope.move_id"
                " FROM account_move_line scope"
                " WHERE scope.analytic_distribution ?| %s)",
                [str(i) for i in analytic_ids],
            ))
        return query

    def _apply_cash_leg_filters(self, query, options, equivalent_ids=()):
        """Option filters for queries over cash legs: the cash-active move
        set, the FX effect and opening / closing cash.

        Journal and partner filters apply to the cash leg, as
        apply_common_filters does. The account filter's cash accounts
        restrict the cash legs; its other accounts and the analytic
        filters select entries (_apply_entry_scope). With no account or
        analytic filter the SQL equals apply_common_filters'.
        """
        options = options or {}
        ...
        cash_ids, entry_ids = self._split_account_filter(
            options, equivalent_ids)
        if cash_ids:
            query.where_accounts(cash_ids)
        self._apply_entry_scope(query, options, entry_ids)
        return query
    ...
    def _fetch_cash_active_move_ids(
        ...
        self._apply_cash_leg_filters(query, options, equivalent_ids)
    ...
    def _fetch_cash_impacts(
        ...
        if options.get('partner_ids'):
            query.where_partners(options['partner_ids'])
        # Analytic and non-cash account filters are not re-applied line by
        # line: they already selected move_ids at entry level, and every
        # counterpart of an in-scope entry is measured so the sections
        # foot to its cash legs.
    ...
    def _fetch_cash_impacts_reconciled(
        ...
        Analytic and account filters already scoped move_ids at entry
        level (_apply_entry_scope); journal/partner filters are applied as
        in _fetch_cash_impacts.
        """
    ...
    def _trace_reconciled_types(self, ar_ap_line):
        """Return {account_type: fraction} of a receivable / payable line's
        cash impact, read from its reconciliation partials back to the
        counterpart moves' income/expense/tax lines.

        * Each partial settles `amount` (company currency, always positive)
          of the line, so its counterpart move receives
          amount / |line balance| of the line, capped at what is left.
        * Within a counterpart move that share is spread over the activity
          lines by SIGNED balance, normalised by their signed sum. Absolute
          weights would inflate offsetting pairs (an anglo-saxon invoice's
          cost of sales against stock interim) and misstate every bucket.
        * Whatever the partials do not trace (an unapplied part of a
          receipt, a counterpart with no activity lines) stays on the
          line's own account type, so the fractions add up to 1.

        Empty dict if the line is unreconciled or nothing traces.
        """
        from collections import defaultdict
        currency = ar_ap_line.company_currency_id
        line_amount = abs(ar_ap_line.balance)
        if currency.is_zero(line_amount):
            return {}
        ...
        for partial in partials.sorted('id'):
            share = min(partial.amount, line_amount - consumed)
            if currency.compare_amounts(share, 0.0) <= 0:
                break
            consumed += share
            counterpart = (
            ...
            signed_total = sum(activity.mapped('balance'))
            if currency.is_zero(signed_total):
                continue
            weight = share / line_amount
            for activity_line in activity:
                if not activity_line.balance:
                    continue
                fractions[activity_line.account_id.account_type] += (
                    weight * activity_line.balance / signed_total)
            traced += share
        if not traced:
            return {}
        if not currency.is_zero(line_amount - traced):
            fractions[ar_ap_line.account_id.account_type] += (
                (line_amount - traced) / line_amount)
        return dict(fractions)

    def _fetch_cash_transfers(
        self, move_ids, company_ids, posted_only, options,
        equivalent_ids=(),
    ):
        """Cash moved between filtered and unfiltered cash accounts.

        Only non-zero when the account filter names cash accounts: their
        entries may carry cash legs on other cash accounts (a bank to bank
        transfer with one side filtered out). Those legs are neither
        activity nor part of the filtered cash position, yet they move it,
        so they are reported on their own line, in the cash impact sign
        convention (-balance: money arriving on the filtered accounts is
        positive). Without a cash account filter every cash leg is in
        scope and this returns 0.0.
        """
        cash_ids = self._split_account_filter(options, equivalent_ids)[0]
        if not move_ids or not cash_ids:
            return 0.0
        query = MoveLineQuery(self.env, company_ids=company_ids)
        if posted_only:
            query.where_posted_only()
        self._apply_cash_account_filter(query, equivalent_ids)
        query.where_raw(SQL("aml.account_id NOT IN %s", tuple(cash_ids)))
        query.where_raw(SQL("aml.move_id IN %s", tuple(move_ids)))
        # Same scope as the counterpart query (_fetch_cash_impacts).
        if options.get('journal_ids'):
            query.where_journals(options['journal_ids'])
        if options.get('partner_ids'):
            query.where_partners(options['partner_ids'])
        query.select(SQL("COALESCE(SUM(-aml.balance), 0)"), 'transfers')
        rows = query.execute()
        if not rows:
            return 0.0
        return round(float(rows[0].get('transfers') or 0.0), 2)
    ...
    def _fetch_cash_balance(
        ...
        The same cash-leg filters (journal / partner / account / analytic,
        see _apply_cash_leg_filters) that scope net_change and fx are
        applied here, so opening and closing measure the identical
        filtered subset. Without this the
    ...
    # Only liabilities can hold amounts owed to the tax authority. Tax
    # repartition lines also target input-tax receivables (assets) and
    # non-deductible tax (expense accounts); a debit on those inside a cash
    # entry is tax paid to a supplier, not a remittance.
    TAX_AUTHORITY_TYPES = ('liability_current', 'liability_payable')
    ...
    def _tax_authority_account_ids(self, company_ids):
        ...
        explicitly to isolate taxes on income. Only repartition targets of
        TAX_AUTHORITY_TYPES qualify.
        """
        ...
        accounts = Repartition.search(domain).mapped('account_id')
        return list(set(accounts.filtered(
            lambda a: a.account_type in self.TAX_AUTHORITY_TYPES).ids))
    ...
    def _resolve_disclosure_accounts(self, company_ids, options=None):
        ...
        accounts = {}
        fallback_items = set()
        # An account is measured under one item at most: under two, the
        # same journal item would leave its bucket twice and show on two
        # lines. The earlier item in DISCLOSURE_ORDER keeps it (interest
        # before tax, as Finance Costs wins over Tax Expense on the Profit
        # and Loss).
        claimed = set()
        for item in self.DISCLOSURE_ORDER:
    ...
    INDIRECT_WORKING_CAPITAL_TYPES = (
        ...
        ('asset_prepayments', _lt("Decrease (Increase) in Prepayments")),
    ...
    def _compute_indirect_operating(
        ...
        * Profit before tax = net income plus the income tax expense
          identified by tagged P&L accounts and, when the income tax
          paid item has a tagged clearing account, the company Tax
          Expense mapping (otherwise the start line is net income). The
          tax expense re-enters through the deltas / disclosed lines, so
          adding it here is presentation, not a gap.
        ...
        * Working-capital deltas are the period balance movement per
        ...
        # Profit-side bridges (tax expense added back to reach profit before
        # tax, finance costs added back, investment income backed out). A
        # bridged amount must re-enter once, through the disclosed line
        # that measures its cash, so:
        # * an account is bridged under one heading at most: finance costs
        #   win over tax (as on the Profit and Loss), and investment income
        #   never takes an account already bridged;
        # * the company P&L mappings (eh_pnl_*_account_ids) join a bridge
        #   only when the matching disclosed item has a tagged clearing
        #   (non-P&L) account (_bridge_account_ids). That tag is what takes
        #   the accrual out of the working-capital deltas; without it the
        #   accrual already reaches operating through the deltas and a
        #   mapped add-back would count it twice.
        fincost_ids = self._bridge_account_ids(
            companies, disclosure_accounts, 'interest_paid',
            'eh_pnl_finance_cost_account_ids')
        tax_account_ids = self._bridge_account_ids(
            companies, disclosure_accounts, 'income_tax_paid',
            'eh_pnl_tax_expense_account_ids') - fincost_ids
        invinc_ids = (
            set(disclosure_accounts.get('interest_received', []))
            | set(disclosure_accounts.get('dividends_received', []))
        ) - fincost_ids - tax_account_ids
    ...
    def _bridge_account_ids(
        self, companies, disclosure_accounts, item, mapping_field,
    ):
        """P&L accounts bridged for one disclosed item (indirect method).

        Always the item's own disclosure accounts. The company mapping
        `mapping_field` joins only when those accounts include a tagged
        clearing (non-P&L) account; see _compute_indirect_operating.
        """
        ids = set(disclosure_accounts.get(item, []))
        if not ids or mapping_field not in companies._fields:
            return ids
        has_clearing = any(
            account.account_type not in self.PL_TYPES
            for account in self.env['account.account'].sudo().browse(
                sorted(ids))
        )
        if has_clearing:
            ids |= set(companies.mapped(mapping_field).ids)
        return ids
