...
class ResPartner(models.Model):
    ...
    def _print_statement(self, xml_id, label):
        ...
        attachment = self.env['ir.attachment'].create({
            ...
            # Not linked to the partner: every user who can read the partner
            # could otherwise download its statement by id. This is a one-off
            # download, so keep it creator-only (unlinked attachment).
            'res_model': False,
            'res_id': False,
        })
        return {
