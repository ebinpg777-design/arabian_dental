...
class EhExecutiveSummaryHandler(models.AbstractModel):
    ...
    # Quick (acid-test) assets: cash and receivables only. Odoo has no
    # inventory account type: stock valuation and raw material accounts sit
    # in asset_current beside other current assets, so asset_current cannot
    # be counted without counting inventory. Prepayments are not quick
    # either.
    QUICK_ASSET_TYPES = ('asset_cash', 'asset_receivable')
    TOTAL_ASSET_TYPES = (
