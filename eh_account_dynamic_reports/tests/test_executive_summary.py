...
class TestExecutiveSummaryHandler(EhAccountIntegrationTestCase):
    ...
    def test_quick_ratio_excludes_inventory(self):
        self._seed_full_set()
        # Inventory bought on credit: Dr Inventory (asset_current) 3000 /
        # Cr Payables 3000. Current, but not a quick asset.
        inventory = self._ensure_account(
            self.env, '1397', 'Inventory', 'asset_current')
        self._post([
            {'account': inventory, 'debit': 3000.0},
            {'account': self.account_payable, 'credit': 3000.0,
             'partner': self.partner_b},
        ])
        result = self.handler.compute(self.options)
        # Current liabilities = AP 4000 + 3000 = 7000.
        # Current ratio = (cash 6000 + AR 4000 + inventory 3000) / 7000.
        current_ratio = self._raw(
            self._line_by_id(result, 'exec-current_ratio'))
        self.assertAlmostEqual(current_ratio, 13000.0 / 7000.0, places=4)
        # Quick ratio = (cash 6000 + AR 4000) / 7000.
        quick_ratio = self._raw(self._line_by_id(result, 'exec-quick_ratio'))
        self.assertAlmostEqual(quick_ratio, 10000.0 / 7000.0, places=4)
