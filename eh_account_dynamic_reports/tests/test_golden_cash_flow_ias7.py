...
class TestCashFlowDisclosuresGolden(CashFlowIas7Case):
    ...
    def test_account_in_two_disclosed_items_is_measured_once(self):
        # An account tagged both Interest Paid and Income Tax Paid is
        # measured under the earlier item only (interest). Before, the same
        # journal item left the Expenses bucket twice and showed on both
        # disclosed lines.
        self._tag(self.account_int_expense, 'account_tag_interest_paid')
        self._tag(self.account_int_expense, 'account_tag_income_tax_paid')
        self._post([
            {'account': self.account_cash, 'debit': 1000.0},
            {'account': self.account_revenue, 'credit': 1000.0},
        ], '2026-02-10')
        ...
        self.assertEqual(
            self._amount(result, 'disclosure-interest_paid'), -50.00)
        self.assertIsNone(self._line(result, 'disclosure-income_tax_paid'))
        self.assertEqual(
            result['totals']['disclosures'], {'interest_paid': -50.00})
        self.assertIsNone(
            self._line(result, 'section-operating-line-expense'))
        self.assertEqual(result['totals']['operating'], 950.00)
        indirect = self._compute(cash_flow_method='indirect')
        self.assertEqual(indirect['totals']['operating'], 950.00)

    def test_account_mapped_as_finance_cost_and_tax_is_bridged_once(self):
        # Interest expense mapped as both a finance cost and a tax expense:
        # the indirect method adds it back once (as a finance cost), so the
        # tie to the direct method holds.
        #   PBT = 830 + 120 (tax only)  = 950.00
        #   + finance costs             =  50.00
        #   = CGO 1000.00; - 50 - 120   = 830.00 = direct
        self._seed_interest_and_tax()
        self.company.sudo().write({
            'eh_pnl_tax_expense_account_ids': [(6, 0, (
                self.account_tax_expense | self.account_int_expense).ids)],
            'eh_pnl_finance_cost_account_ids': [
                (6, 0, self.account_int_expense.ids)],
        })
        indirect = self._compute(cash_flow_method='indirect')
        self.assertEqual(self._amount(indirect, 'indirect-pbt'), 950.00)
        self.assertEqual(
            self._amount(indirect, 'indirect-adj-finance-costs'), 50.00)
        self.assertEqual(indirect['totals']['operating'], 830.00)
        direct = self._compute()
        self.assertEqual(
            direct['totals']['operating'], indirect['totals']['operating'])

    def test_tax_mapping_without_tagged_clearing_account_is_not_bridged(self):
        # Tax expense mapped on the company, nothing tagged: the accrual
        # stays in the working-capital deltas, so adding the tax back to
        # profit as well would count it twice.
        #   direct: +1000 sale - 120 tax settled   = 880.00
        #   indirect: start 880 (no bridge) + payable delta 0 = 880.00
        self.company.sudo().eh_pnl_tax_expense_account_ids = [
            (6, 0, self.account_tax_expense.ids)]
        self._post([
            {'account': self.account_cash, 'debit': 1000.0},
        ...
        direct = self._compute()
        indirect = self._compute(cash_flow_method='indirect')
        self.assertEqual(direct['totals']['operating'], 880.00)
        self.assertEqual(self._amount(indirect, 'indirect-pbt'), 880.00)
        self.assertEqual(indirect['totals']['operating'], 880.00)

    def test_income_tax_fallback_ignores_input_tax_asset_accounts(self):
        # Opted-in fallback with a purchase tax whose repartition targets an
        # input-tax RECEIVABLE (asset). Tax paid to a supplier inside a cash
        # purchase is not a remittance and must not surface as income taxes
        # paid; the liability target is still picked up.
        authority_payable = self._seed_tax_authority_fixture()
        input_vat = self._ensure_account(
            self.env, '1297', 'Input Tax Receivable', 'asset_current')
        sale_tax = self.env['account.tax'].search([
            ('name', '=', 'EH IAS7 Fallback Tax'),
            ('company_id', '=', self.company.id),
        ], limit=1)
        input_tax = self.env['account.tax'].create({
            'name': 'EH IAS7 Input Tax',
            'amount': 15.0,
            'amount_type': 'percent',
        ...
        (input_tax.invoice_repartition_line_ids
         | input_tax.refund_repartition_line_ids).filtered(
            lambda l: l.repartition_type == 'tax',
        ).write({'account_id': input_vat.id})
        fallback_ids = self.handler._tax_authority_account_ids(
            [self.company.id])
        self.assertIn(authority_payable.id, fallback_ids)
        self.assertNotIn(input_vat.id, fallback_ids)

        # Cash purchase: Dr Expense 100, Dr Input Tax 15 / Cr Cash 115.
        self._post([
            {'account': self.account_expense, 'debit': 100.0},
            {'account': input_vat, 'debit': 15.0},
            {'account': self.account_cash, 'credit': 115.0},
        ])
        result = self._compute(cf_income_tax_fallback=True)
        self.assertIsNone(
            self._line(result, 'disclosure-income_tax_paid'),
        ...
        self.assertNotIn('disclosures', result['totals'])
        self.assertEqual(
            self._amount(result, 'section-operating-line-asset_current'),
            -15.00)
        self.assertEqual(result['totals']['operating'], -115.00)
