...
class TestDeferredScheduleHandler(EhAccountIntegrationTestCase):
    ...
    # ---- drilldown scoping (independent of eh.asset) ----

    def test_drilldown_domain_mirrors_posted_only_and_company(self):
        date_to = date(2026, 12, 31)
        company_ids = [self.company.id]
        posted = self.rev_handler._drilldown_move_domain(
            dict(self.options, posted_only=True), [1, 2], company_ids,
            date_to)
        self.assertIn(('move_id', 'in', [1, 2]), posted)
        self.assertIn(('company_id', 'in', company_ids), posted)
        self.assertIn(('date', '<=', '2026-12-31'), posted)
        self.assertIn(('parent_state', '=', 'posted'), posted)

        everything = self.rev_handler._drilldown_move_domain(
            dict(self.options, posted_only=False), [1, 2], company_ids,
            date_to)
        self.assertIn(('company_id', 'in', company_ids), everything)
        self.assertNotIn(('parent_state', '=', 'posted'), everything)
        self.assertIn(('parent_state', '!=', 'cancel'), everything)

        # posted_only absent: same default as compute() (True).
        options = dict(self.options)
        options.pop('posted_only')
        self.assertTrue(self.rev_handler.compute(options)['meta'][
            'posted_only'])
        self.assertIn(
            ('parent_state', '=', 'posted'),
            self.rev_handler._drilldown_move_domain(
                options, [1], company_ids, date_to))

    def test_asset_scope_domain_bounds_type_and_company(self):
        company_ids = [self.company.id]
        rev = self.rev_handler._asset_scope_domain(company_ids)
        exp = self.exp_handler._asset_scope_domain(company_ids)
        self.assertIn(('deferred_type', '=', 'deferred_revenue'), rev)
        self.assertIn(('deferred_type', '=', 'deferred_expense'), exp)
        self.assertIn(('company_id', 'in', company_ids), rev)
        self.assertIn(('company_id', 'in', company_ids), exp)
