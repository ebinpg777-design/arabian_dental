...
class TestCashFlowHandler(EhAccountIntegrationTestCase):
    ...
    # ---- account / analytic filter scoping ----

    def test_cash_account_filter_reports_transfer_out_of_scope(self):
        # Filtering on one bank account: a transfer in from another cash
        # account is not activity, but it moves the filtered cash. It is
        # reported on its own line and the Balance Check stays at zero.
        bank = self._ensure_account(
            self.env, '1019', 'Filter Bank', 'asset_cash',
        )
        self._post_in_period([
            {'account': bank, 'debit': 500.0},
            {'account': self.account_cash, 'credit': 500.0},
        ...
        self._post_in_period([
            {'account': bank, 'debit': 200.0},
            {'account': self.account_revenue, 'credit': 200.0},
        ], date_str='2026-06-20')
        result = self.handler.compute(dict(self.options, account_ids=[bank.id]))
        totals = result['totals']
        self.assertAlmostEqual(totals['operating'], 200.0, places=2)
        self.assertAlmostEqual(totals['net_change_in_cash'], 200.0, places=2)
        self.assertAlmostEqual(
            totals['cash_transfers_outside_filter'], 500.0, places=2)
        self.assertAlmostEqual(
            self._amount(self._line_by_id(
                result, 'cash_transfers_outside_filter')),
            500.0, places=2)
        self.assertAlmostEqual(
            totals['closing_cash_balance'] - totals['opening_cash_balance'],
            700.0, places=2)
        self.assertAlmostEqual(totals['balance_check'], 0.0, places=2)
        # Unfiltered, the transfer is a pure cash movement: no line, no key.
        unfiltered = self.handler.compute(self.options)
        self.assertIsNone(self._line_by_id(
            unfiltered, 'cash_transfers_outside_filter'))
        self.assertNotIn(
            'cash_transfers_outside_filter', unfiltered['totals'])
        self.assertAlmostEqual(
            unfiltered['totals']['balance_check'], 0.0, places=2)

    def test_non_cash_account_filter_selects_entries(self):
        # A non-cash account in the filter selects the entries touching it;
        # it used to be matched against the cash legs and empty the report.
        self._post_in_period([
            {'account': self.account_cash, 'debit': 300.0},
            {'account': self.account_revenue, 'credit': 300.0},
        ])
        ...
        result = self.handler.compute(
            dict(self.options, account_ids=[self.account_revenue.id]))
        totals = result['totals']
        self.assertAlmostEqual(totals['operating'], 300.0, places=2)
        self.assertAlmostEqual(totals['financing'], 0.0, places=2)
        self.assertAlmostEqual(totals['net_change_in_cash'], 300.0, places=2)
        self.assertAlmostEqual(
            totals['closing_cash_balance'] - totals['opening_cash_balance'],
            300.0, places=2)
        self.assertAlmostEqual(totals['balance_check'], 0.0, places=2)

    def _post_cash_expense_with_analytic(self, amount, analytic, date_str):
    ...
    def test_analytic_filter_scopes_entries_not_cash_legs(self):
        # Cash legs never carry an analytic distribution. The filter selects
        # entries whose lines carry it, so the statement is not emptied and
        # the Balance Check identity still holds.
        plan = self.env['account.analytic.plan'].create(
            {'name': 'Cash Flow Filter Plan'})
        project_x = self.env['account.analytic.account'].create(
            {'name': 'CF Project X', 'plan_id': plan.id})
        project_y = self.env['account.analytic.account'].create(
            {'name': 'CF Project Y', 'plan_id': plan.id})
        self._post_cash_expense_with_analytic(100.0, project_x, '2026-06-15')
        self._post_cash_expense_with_analytic(40.0, project_y, '2026-06-18')
        # No analytic at all: out of scope under any analytic filter.
        self._post_in_period([
            {'account': self.account_cash, 'debit': 1000.0},
            {'account': self.account_revenue, 'credit': 1000.0},
        ], date_str='2026-06-20')

        by_account = self.handler.compute(
            dict(self.options, analytic_account_ids=[project_x.id]))
        totals = by_account['totals']
        self.assertAlmostEqual(totals['operating'], -100.0, places=2)
        self.assertAlmostEqual(totals['net_change_in_cash'], -100.0, places=2)
        self.assertAlmostEqual(
            totals['closing_cash_balance'] - totals['opening_cash_balance'],
            -100.0, places=2)
        self.assertAlmostEqual(totals['balance_check'], 0.0, places=2)

        by_plan = self.handler.compute(
            dict(self.options, analytic_plan_ids=[plan.id]))
        self.assertAlmostEqual(
            by_plan['totals']['operating'], -140.0, places=2)
        self.assertAlmostEqual(
            by_plan['totals']['balance_check'], 0.0, places=2)

    # ---- prepayments are working capital ----

    def test_prepayment_is_operating_in_both_methods(self):
        prepaid = self._ensure_account(
            self.env, '1497', 'Prepaid Insurance', 'asset_prepayments',
        )
        self._post_in_period([
            {'account': prepaid, 'debit': 1200.0},
            {'account': self.account_cash, 'credit': 1200.0},
        ])
        direct = self.handler.compute(self.options)
        self.assertAlmostEqual(
            direct['totals']['operating'], -1200.0, places=2)
        self.assertAlmostEqual(direct['totals']['investing'], 0.0, places=2)
        self.assertAlmostEqual(
            self._amount(self._line_by_id(
                direct, 'section-operating-line-asset_prepayments')),
            -1200.0, places=2)
        self.assertIsNone(self._line_by_id(
            direct, 'section-investing-line-asset_prepayments'))

        indirect = self.handler.compute(
            dict(self.options, cash_flow_method='indirect'))
        self.assertAlmostEqual(
            indirect['totals']['operating'], -1200.0, places=2)
        self.assertAlmostEqual(
            self._amount(self._line_by_id(
                indirect, 'indirect-wc-asset_prepayments')),
            -1200.0, places=2)
        self.assertAlmostEqual(
            indirect['totals']['balance_check'], 0.0, places=2)

    # ---- reconciliation-accurate weights ----

    def test_reconciled_weights_follow_partial_amounts(self):
        # One receipt of 1000 settles invoice A (income 300) in full and
        # 700 of invoice B (other income 10000). Each invoice gets what it
        # settled, not a share of its own size.
        other_income = self._ensure_account(
            self.env, '4197', 'Other Operating Income', 'income_other',
        )
        inv_a = self._post_in_period([
            {'account': self.account_receivable, 'debit': 300.0,
             'partner': self.partner_a},
            {'account': self.account_revenue, 'credit': 300.0},
        ], date_str='2026-03-01')
        inv_b = self._post_in_period([
            {'account': self.account_receivable, 'debit': 10000.0,
             'partner': self.partner_a},
            {'account': other_income, 'credit': 10000.0},
        ], date_str='2026-03-02')
        pay = self._post_in_period([
            {'account': self.account_cash, 'debit': 1000.0},
            {'account': self.account_receivable, 'credit': 1000.0,
        ...
        (ar(inv_a) + ar(pay)).reconcile()
        (ar(inv_b) + ar(pay)).reconcile()
        ...
        self.assertAlmostEqual(
            self._amount(self._line_by_id(
                fine, 'section-operating-line-income_other')),
            700.0, places=2)
        self.assertIsNone(self._line_by_id(
            fine, 'section-operating-line-asset_receivable'))
        self.assertAlmostEqual(fine['totals']['operating'], 1000.0, places=2)
    ...
    def test_reconciled_unapplied_receipt_stays_in_receivables(self):
        inv = self._post_in_period([
            {'account': self.account_receivable, 'debit': 600.0,
             'partner': self.partner_a},
            {'account': self.account_revenue, 'credit': 600.0},
        ])
        pay = self._post_in_period([
        ...
        (inv.line_ids + pay.line_ids).filtered(
            lambda l: l.account_id == self.account_receivable).reconcile()
        ...
        self.assertAlmostEqual(
            self._amount(self._line_by_id(
                fine, 'section-operating-line-income')),
            600.0, places=2)
        # The 400 not applied to any invoice is still a receivable receipt.
        self.assertAlmostEqual(
            self._amount(self._line_by_id(
                fine, 'section-operating-line-asset_receivable')),
            400.0, places=2)
    ...
    def test_reconciled_signed_weights_net_offsetting_lines(self):
        # Anglo-saxon shaped invoice: revenue 1000 plus cost of sales 600
        # against stock interim 600. The offsetting pair nets out, so the
        # full receipt stays attributed to income.
        stock_interim = self._ensure_account(
            self.env, '1397', 'Stock Interim (Delivered)', 'asset_current',
        )
        cogs = self._ensure_account(
            self.env, '5197', 'Cost of Goods Sold', 'expense_direct_cost',
        )
        inv = self._post_in_period([
            {'account': self.account_receivable, 'debit': 1000.0,
             'partner': self.partner_a},
            {'account': self.account_revenue, 'credit': 1000.0},
            {'account': cogs, 'debit': 600.0},
            {'account': stock_interim, 'credit': 600.0},
        ])
        pay = self._post_in_period([
            {'account': self.account_cash, 'debit': 1000.0},
            {'account': self.account_receivable, 'credit': 1000.0,
