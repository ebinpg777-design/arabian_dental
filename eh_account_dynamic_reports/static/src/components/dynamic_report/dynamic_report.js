...
export class EhDynamicReportViewer extends Component {
    ...
    async loadFilterChoices() {
        ...
        const [companies, journals, partners, accounts, plans, analyticAccounts, currencies, teams] = await Promise.all([
            ...
            this.tryCall("eh.account.dynamic.report.handler.aged_receivable",
                         "eh_sales_routes", [], {}),
        ]);
        this.state.choices.teams = teams || [];
    ...
    async tryCall(model, method, args, kwargs) {
        // Same tolerance as tryFetch, for a server helper: a missing model or a
        // rights problem must leave the filter empty, never break the screen.
        try {
    ...
    onGroupByRouteToggle(event) {
        this.state.options.group_by_route = event.target.checked;
        this.refresh();
    }
    ...
    activeChips() {
        ...
        const named = [
            ...
            ["team_ids", this.state.choices.teams || [], "Sales Route"],
        ...
        if (this.isAged) {
            ...
            if (opts.group_by_route) {
                chips.push({ label: "Grouped by route", key: "group_by_route" });
            }
