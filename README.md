...
in the corner. They are generated by `tools/make_icons.py`, so a new module gets one by
adding a line to it. See **The brand** below.
...
Everything visual comes from the lab's own brand sheet, committed at
`lab_website/static/src/img/brand/` as the PDF, its vector original and a 600 dpi
master PNG.

| | |
| --- | --- |
| Red | `#ed1c24` (CMYK 0 100 100 0) |
| Grey | `#606a70` (CMYK 10 0 0 70) |
| Logo | shield + "Arabian Dental Lab", with "Since 2000" |

Two scripts cut everything else from that master, and both are re-runnable:

* `tools/make_brand_assets.py` — the horizontal and stacked lock-ups, the shield
  alone, the wordmark alone, white versions for dark backgrounds, the square app
  icons and the home screen's wordmarks. White versions are made by hue: anything
  with no colour in it goes white and the red is left as specified, because
  inverting the artwork gives a cyan tooth.
* `tools/make_icons.py` — the app icons of the lab's own modules: the brand red
  for what the lab sells and its customers touch, the brand grey for what the lab
  runs on, a white Font Awesome pictogram (the same symbols the menus use) and the
  shield in the corner. `epg_*`, `eh_*` and the third-party add-ons keep their own
  icons: they are sold to other customers.

Odoo's own purple and teal are left alone in the backend on purpose. The brand is
carried by the logo, the icons and the wallpaper; turning every primary button red
as well made an ordinary save look like a warning.

## The home screen

`lab_home` takes over the screen every user lands on.

**The wallpaper.** Four are shipped, each composed by
`tools/make_home_wallpaper.py` and committed beside the module:
...
The bare CSS rule is a light picture whatever the default is set to, because the
captions only turn white under a class, and a dark picture arriving without that
class would leave near-black app names on a near-black screen.
...
Only users who can act on a warning are shown it; today that means Settings
...
One trap worth keeping: Odoo puts `o_home_menu_background` on `<body>` itself and
the wallpaper class goes on that same element, so a descendant selector matches
nothing. Each variant is written both compounded and as a descendant.
